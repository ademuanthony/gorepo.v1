...
func (gr GormRepository) Insert(model IModel) (interface{}, error){
...
func (gr GormRepository) Save(model IModel) (interface{}, error){
	if err := model.Validate(); err != nil{
		return 0, err
	}
...
func (gr GormRepository) FindById(receiver IModel, id interface{}) (error){
...
func (gr GormRepository) Count(models interface{}, where string, args ...interface{}) (result int, err error){
	err = gr.Db.Model(models).Where(where, args...).Count(&result).Error
	return
}

func (gr GormRepository) FindAllWithLimit(models interface{}, offset int, limit int, where string, args ...interface{}) (err error){
	err = gr.Db.Where(where, args...).Offset(offset).Limit(limit).Find(models).Error
	return
}
...
func (gr GormRepository) RawSql(sql string, value ...interface{}) (db *gorm.DB) {
	db = gr.Db.Debug().Raw(sql, value...)
	return
}
